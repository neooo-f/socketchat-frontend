...
export const login = async (request: { username: string, password: string }) => {
    ...
    return fetch<any>('auth/login', 'POST', request, undefined, undefined, undefined, undefined, false);
}
...
export const register = async (newUser: Omit<UserType, "id">, file?: File) => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return fetch<any>('auth/register', 'POST', formData, undefined, undefined, undefined, undefined, false)
}